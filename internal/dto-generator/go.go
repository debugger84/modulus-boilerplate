...
func (f *Funcs) FuncMap() template.FuncMap {
	return template.FuncMap{
		...
		"field":        f.field,
		"short":        f.short,
		"hasEqQuery":   f.hasEqQuery,
		"hasInQuery":   f.hasInQuery,
		"hasLikeQuery": f.hasLikeQuery,
		// sqlstr funcs
		"querystr": f.querystr,
		"sqlstr":   f.sqlstr,
		// helpers
...
// context_both returns true with the context mode is both.
func (f *Funcs) slice_strings(v interface{}) bool {
	switch x := v.(type) {
	case Field:
		return x.Type == "[]sql.NullString"
...
func (f *Funcs) field(field Field, structName string) (string, error) {
	...
	if s := buf.String(); s != "" {
		gormType := ""
		if fields, ok := types[structName]; ok {
			if changedType, ok := fields[field.GoName]; ok {
				if changedType.DbType != "" {
					gormType = ";type:" + changedType.DbType
				}
			}
		}
		tag = "`" + `gorm:"column:` + field.SQLName + gormType + `" json:"` +
			f.snakeCaseToCamelCase(field.SQLName) + `"` + "`"
	}

	if fields, ok := types[structName]; ok {
		if changedType, ok := fields[field.GoName]; ok {
			if changedType.GoType != "" {
				field.Type = changedType.GoType
			}
		}
	}
...
func (f *Funcs) snakeCaseToCamelCase(inputUnderScoreStr string) (camelCase string) {
...
func (f *Funcs) hasEqQuery(typeName string) bool {
	...
	if _, ok := simpleTypes[typeName]; ok {
		return true
	}
	if _, ok := simpleTypes[strings.TrimLeft(typeName, "*")]; ok {
		return true
	}
	return false
}

func (f *Funcs) hasLikeQuery(typeName string) bool {
	likeTypes := map[string]bool{
		"string": true,
	}
	if _, ok := likeTypes[typeName]; ok {
		return true
	}
	if _, ok := likeTypes[strings.TrimLeft(typeName, "*")]; ok {
		return true
	}
	return false
}

func (f *Funcs) hasInQuery(typeName string) bool {
	likeTypes := map[string]bool{
		"uuid.UUID": true,
		"string":    true,
		"int":       true,
